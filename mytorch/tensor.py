...
class Tensor:
    ...
    def is_leaf(self):
        """
        判断当前 Tensor 是否为叶子节点。
        叶子节点是指由用户直接创建的 Tensor（例如模型权重、输入数据），其 creator 为 None。
        """
    ...
    def _check_inplace_safe(self):
        """
        原地操作安全拦截器。
        严格禁止对需要求导的非叶子节点（中间运算结果）进行 inplace 操作。
        """
        if self.requires_grad and not self.is_leaf:
            raise RuntimeError(
                "RuntimeError: 试图对需要求导的内部节点（非叶子节点）进行 inplace 操作 "
                "（如 add_, mul_ 等）。这会覆盖前向传播的中间缓存，破坏计算图，"
                "导致反向传播计算出错误的梯度。请使用 out = a + b 替代 a.add_(b)。"
            )
    ...
    def __add__(self, other):
        """重载 + 运算符 (self + other)"""
        # 如果 other 不是 Tensor（例如是数字），先将其包装为 Tensor
        if not isinstance(other, Tensor):
            # 保持与当前 Tensor 在同一设备上
            other = Tensor(other, device=self._device)
        
        # 局部导入 Add 算子，防止与 function.py 产生循环引用
        from .function import Add
        # 调用 Add 算子进行前向计算，并自动构建计算图
        return Add()(self, other)

    def __radd__(self, other):
        """处理反向加法 (other + self)，例如 5 + tensor"""
        return self.__add__(other)

    def __iadd__(self, other):
        """处理原地加法 (self += other)"""
        # 直接复用你已经实现的 add_ 方法
        # 注意：add_ 内部有 _check_inplace_safe() 检查
